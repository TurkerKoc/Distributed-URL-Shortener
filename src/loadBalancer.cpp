...
Status LoadBalancer::Write(ServerContext *context, const WriteRequest *req, WriteResponse *res) {
    ...
    for (int i = 0; i < (int) (nodesClientService.size()); i++) {
        ...
        Status status = (nodesClientService[(unsigned long) writeOrder])->Write(&clientContext, requestToNode,
                                                                                &responseFromNode); //sending request to node with writeorder index
...
Status LoadBalancer::Read(ServerContext *context, const ReadRequest *req, ReadResponse *res) {
    ...
    int readQuorum = (int) std::ceil(
            ((int) (nodesClientService.size())) / (double) 2); //division round up to get quorum

    for (int i = 0; i < readQuorum; i++) {
        std::cout << "Read request to node " << readOrder << " with url: " << url << " with read quorum: " << readQuorum
                  << std::endl;
        ReadResponse responseFromNode;
        ClientContext clientContext;
        Status status = (nodesClientService[(unsigned long) readOrder])->Read(&clientContext, requestToNode,
                                                                              &responseFromNode);
        readOrder++;
